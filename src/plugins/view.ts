...
export class PluginsView extends AbstractPlugins {
  ...
  private initialize() {
    ...
    p.innerHTML = '请选择:.mp4(视频格）的文件，最大不可以超过100MB,请选择:.mp4(视频格）的文件，最大不可以超过100MB'
  ...
  public uploadFile(files: Array<File>) {
    if (this.uploader) {
      this.uploader.upload(files.map(item => {
        return {
          name: item.name, size: item.size, file: item
        }
      }))
    }
  }
    ...
    switch (evt.type) {
      ...
      case 'paste':
        ...
        if (files.length) {
          ...
          AUpload.Message.warn(`粘帖上传时未找到可用文件，请重试`, { autoClose: true })
        }
        break;
    }
  }
